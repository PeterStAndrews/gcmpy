...
class NetworkToEdgeList:
    ...
    def convert(network: Network) -> LightWeightEdgeList:
        ...
        model.joint_degrees = [network.G.nodes[n][NetworkNames.JOINT_DEGREE] for n in range(len(network.G.nodes()))]
        model.topologies = [network.G.edges[e][NetworkNames.TOPOLOGY] for e in network.G.edges()]
        model.motif_id = [network.G.edges[e][NetworkNames.MOTIF_IDS] for e in network.G.edges()]
        return model