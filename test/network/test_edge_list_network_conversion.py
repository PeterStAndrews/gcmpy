...
class ConversionTests(unittest.TestCase):
    def test_conversions(self):
        ...
        g2: Network = EdgeListToNetwork.convert(edge_list)

        # test equality
        for e in g._G.edges():
            self.assertTrue(g2.G.has_edge(*e))
            self.assertTrue(
                g.G.edges[e][NetworkNames.TOPOLOGY]
                == g2.G.edges[e][NetworkNames.TOPOLOGY]
            )
            self.assertTrue(
                g.G.edges[e][NetworkNames.MOTIF_IDS]
                == g2.G.edges[e][NetworkNames.MOTIF_IDS]
            )

        for n in g.G.nodes():
            self.assertTrue(
                g.G.nodes[n][NetworkNames.JOINT_DEGREE]
                == g2.G.nodes[n][NetworkNames.JOINT_DEGREE]
            )
